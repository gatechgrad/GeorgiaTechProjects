...
class Ship {
  ...
  public void drawShipView(Graphics g, int x, int y, ImageObserver theObserver) {
    ...
    if ( (iLevel < 4) && (iLevel >= 0)) {
      g.setColor( new Color((iLevel * 4) + 128, 0, 0));  //reds
    } else if ( (iLevel < 12) && (iLevel >= 4)) {
      iLevel -= 4;
      g.setColor( new Color((iLevel * 8) + 128, (iLevel * 8) + 128, 0));  //yellows

    } else if ( (iLevel < 140) && (iLevel >= 12)) {
      iLevel -= 12;
      g.setColor( new Color(0, 0, (iLevel * 128) + 128));  //blues


    } else if ( (iLevel < 268) && (iLevel >= 140)) {
      iLevel -= 140;
      g.setColor( new Color(0, (iLevel * 128) + 128, 0));  //greens
  ...
  /**
    * drawItemView
    **/
  public void drawItemView(Graphics g, int x, int y, ImageObserver theObserver) {
    int iMagnitude;
    ...
    if (iFighters > 0) {
      g2.drawString("Fighters: " + iFighters, x + 5, y2 += g.getFont().getSize());
    }
    if (iArmidMines > 0) {
      g2.drawString("Armid Mines: " + iArmidMines, x + 5, y2 += g.getFont().getSize());
    }
    if (iLimpetMines > 0) {
      g2.drawString("Limpet Mines: " + iLimpetMines, x + 5, y2 += g.getFont().getSize());
    }
    if (iGTorps > 0) {
      g2.drawString("G Torpedoes: " + iGTorps, x + 5, y2 += g.getFont().getSize());
    }
    if (iPhotons > 0) {
      g2.drawString("Photon Missiles: " + iPhotons, x + 5, y2 += g.getFont().getSize());
    }
    if (iAtomicDetonators > 0) {
      g2.drawString("Atomic Detonators: " + iAtomicDetonators, x + 5, y2 += g.getFont().getSize());
    }
    if (iEtherProbes > 0) {
      g2.drawString("Ether Probes: " + iEtherProbes, x + 5, y2 += g.getFont().getSize());
    }
    if (iMineDisruptors > 0) {
      g2.drawString("Mine Disruptors: " + iMineDisruptors, x + 5, y2 += g.getFont().getSize());
    }
